...
class Graph():
	...
	def __getattr__(self, attr: "non-existed attribute"):
		raise AttributeError("Graph nave no attribute \"{0}\"".format(attr))
	...
	@staticmethod
	def check_vertex(vertex: "unmutable"):
		if type(vertex) != str and type(vertex) != int:
			raise TypeError("Graph vertex must be represented as a string or an integer")
	...
	@staticmethod
	def edge_to_graph(edge: "tuple of 2 vertices"):
